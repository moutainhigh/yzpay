...
<div class="bjui-pageHeader"><br>
    <form id="pagerForm" data-toggle="ajaxsearch" method="post" action="${baseURL}/sys/merchant/approve">
        <div class="bjui-searchBar">
            <label>商户号：</label><input type="text" name="merchantNo" value="${requestScope.merchantNo}" size="15" alt="模糊查询" class="form-control" />&nbsp;
            ...
            <div>
            	<button type="button" class="btn btn-green" href="${baseURL}/sys/merchant/payConfig?merchantNo={#bjui-selected}" title="支付配置" data-toggle="dialog" data-width="800" data-height="650" data-id="dialog-normal">支付配置</button>
				...
				<tr target="merch_id" data-id="${item.merchantNo}">
					...
					<td align="center">
						<c:if test="${item.auditStatus != 1}">
					     <button type="button" class="btn btn-green" href="${baseURL}/sys/merchant/approvePage?merchantNo=${item.merchantNo}" 
					     title="商户审核" data-toggle="dialog"  data-width="740"  data-height="650" data-id="dialog-normal">审核</button>
					    </c:if>
						<c:if test="${item.auditStatus == 1}">
					     <button type="button" class="btn btn-green" title="商户审核" data-toggle="dialog"  data-width="740"  data-height="650" 
					     data-id="dialog-normal" style="color: gray;border-color: gray;">审核</button>
					    </c:if>
					</td>
				</tr>
			</c:forEach>
