...
<div class="pageContent">
	<form id="form" method="post" action="${baseURL}/sys/user/add" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone);">
			...
			<p style="width:99%">
				...
				<select name="status" class="required combox">
					...
					<c:forEach items="${UserStatusEnumList}" var="userStatus">
						<option value="${userStatus.name}"
							<c:if test="${status ne null and status eq userStatus.name}">selected="selected"</c:if>>
							${userStatus.desc}
						</option>
					</c:forEach>
				</select>
