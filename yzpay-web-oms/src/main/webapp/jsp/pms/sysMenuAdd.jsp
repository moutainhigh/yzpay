...
<div class="pageContent">
	<form id="form" method="post" action="${baseURL }/sys/menu/add" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone);">
		<div class="pageFormContent" layoutH="60">
			...
			<p style="width: 99%">
				<label>上级菜单：</label>
				<input type="text" name="parent.name" value="${sysMenu.parent.name}" readonly="true" />
			...
			<p style="width: 99%">
				<label>菜单名称：</label>
				<input type="text" name="name" class="required" maxlength="90" value="${sysMenu.name}" />
			</p>
			<p style="width: 99%">
				<label>菜单编号：</label>
				<input type="text" name="number" class="required number" maxlength="20" value="${sysMenu.number}" />
			</p>
			<p style="width: 99%">
				<label>请求URL：</label>
