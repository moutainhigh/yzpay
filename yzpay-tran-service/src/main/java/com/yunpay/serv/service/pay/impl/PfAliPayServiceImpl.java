...
/**
 * 文件名称:    PfAliPayServiceImpl.java
 * 内容摘要:    浦发渠道支付宝支付接口业务核心处理类（条码支付、扫码支付）
 * @author:   Zeng Dongcheng
 * @version:  1.0  
 * @Date:     2017年10月11日下午3:46:33 
 * 
 * 修改历史:  
 * 修改日期                     修改人员                                   版本	            修改内容  
 * ----------------------------------------------  
 ...
 * 公司:   深圳市至高通信技术发展有限公司
 */
@Service("pfAliPayService")
public class PfAliPayServiceImpl extends PfQrPayService{
	...
	/**
	 * @Description:浦发银行支付宝条码支付
	 * @author:   Zeng Dongcheng
	 * @Date:     2017年10月11日下午3:50:27 
	 * @param qrPayReq
	 * @param merchant
	 * @return
	 */
	...
	/**
	 * @Description:浦发银行支付宝扫码支付
	 * @author:   Zeng Dongcheng
	 * @Date:     2017年10月11日下午3:51:18 
	 * @param qrPayReq
	 * @param merchant
	 * @return
	 */
	...
	public Message doScanPay(ScanPayReqDto scanPayReq, Merchant merchant) {
		...
		try {
			...
			PayTranLs payTranLs = payTranLsService.createScanTranLs(scanPayReq, merchant, 
					SubChannel.ALIPAY_SCAN,Route.PF_BANK);
			//金额转换(元转为分)
			String totalFee = AmountUtil.changeY2F(payTranLs.getTransPrice().toString());
			//获取本地ip
			String ip = InetAddress.getLocalHost().getHostAddress();
			PfPayReq aliScanReq = new PfPayReq(
					SubChannel.ALIPAY_SCAN,ConfigUtils.AGENT_MERCHANT_NO,
					ip,pfbankConfig.getChannelMerNo(),pfbankConfig.getAliMchId(),payTranLs.getTransNum(),
					PayConfig.PFBANK_PAY_NOTIFY_URL,totalFee,scanPayReq.getBody(),null);
			//发起支付请求
			PfScanRep rep = pfPayService.doPfScanPay(aliScanReq);
			if(rep!=null){
				if("0000".equals(rep.getRespCode())){
					//订单创建成功
